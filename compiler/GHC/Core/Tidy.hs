...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- Before tidy, ids of function arguments which have a call-by-value semantics are identified
-- and given an `OtherCon[]` unfolding. During tidy, we transform this information into CBV marks
-- that are then associated with the function id itself.
-- Then the code generator only has to query the CBV marks and doesn't need to query any unfolding.
...
-- If the id is boot-exported we don't set a cbv calling convention via marks,
...
-- as boot files don't have unfoldings.
