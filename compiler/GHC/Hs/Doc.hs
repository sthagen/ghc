...
instance Outputable Docs where
  ppr docs =
      vcat
        ...
        , pprField (pprMap ppr (ppr . map pprHsDocDebug)) "declaration docs" docs_decls
        , pprField (pprMap ppr (pprIntMap ppr pprHsDocDebug)) "arg docs" docs_args
...
data ExtractedTHDocs =
  ExtractedTHDocs
    ...
    , ethd_decl_docs  :: Map Name (HsDoc GhcRn)
      -- ^ The documentation added to declarations.
    , ethd_arg_docs   :: Map Name (IntMap (HsDoc GhcRn))
      -- ^ The documentation added to function arguments.
    , ethd_inst_docs  :: Map Name (HsDoc GhcRn)
