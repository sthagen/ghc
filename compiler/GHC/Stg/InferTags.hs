...
module GHC.Stg.InferTags ( inferTags, collectExportInfo ) where
...
import Data.Maybe
import GHC.Stg.InferTags.TagSig (TagInfo(TagProper))
...
* Do the first pass which is an analysis to get `GenStgTopBinding 'InferTaggedBinders`.
...
* Then code generation uses this information to skip the thunk check when branching on/evaluating
...
it made the code simpler. But besides complexity there isn't any reason why we couldn't be more rigourous
in dealing with functions.
